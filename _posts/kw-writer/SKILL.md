...
name: kw-writer
description: |
  Koutian Wu (kw) 专属的博客与长文写作skill。融合了个人双语博客（中英双语输出、前端YAML格式、作者署名规范等）的强制要求。当用户要求写博客（blog）、写公众号、或者明确要求用kw-writer风格写作双语文章时使用。包含L1-L4的行文检测，以及严格的格式化规则（包括第一句话和作者署名等）。
---
