...
**"有见识的普通人在认真聊一件打动他的事，同时严格遵守博客的排版与双语输出规范。"**
...
**真诚是唯一的捷径。** 可以不写，但绝不骗人。如果产品有缺点就直接说，不懂就大方承认。读者的信任是最宝贵的资产。保持直接、实用和犀利的语气，拒绝空洞的鸡汤和企业公关黑话。
...
把深度建立在**因果链、证据、数字含义、反例和适用边界**上，不要把缩写数量、术语密度或长句当成深度。面向非专业读者、小白或儿童写作时，降低理解门槛，但不得删掉决定结论是否成立的限定条件。

涉及金融、法律、医学、科研、工程、政策或其他专业概念时，必须遵守：

1. **先讲问题，再给术语。** 先用日常语言说明这个概念在回答什么问题，再给正式名称。不要用一个陌生术语定义另一个陌生术语。
2. **缩写和术语首次出现必须当场过三关。** 写出全称、自然中文名和一句话作用，不能先使用、隔几段再补课。标题、导语、小标题、图注、表格和正文都算出现位置；如果标题必须保留广告原文或缩写，就在标题或紧接标题的第一句话完成解释。DBS、SIN、MAS、CET1、LCR、HQLA、NSFR、NPL、uptime 之类的行业常用词，对普通读者仍可能是黑话。
3. **递归拆掉黑话。** 定义里如果又出现「风险加权资产」「高质量流动资产」「压力情景」等新概念，继续解释，直到目标读者不需要额外背景也能复述。只把中文术语换成英文全称不算解释。
4. **百分比必须交代谁除以谁。** 明确分子、分母、单位和时间范围，再用每100元或每100个风险单位做数值翻译。紧接着说明这个比例不代表什么，防止读者把 LCR 当成现金占全部存款的比例。
5. **情景必须写成会发生什么。** 解释压力测试、模型假设或实验条件时，写清谁设定情景、哪些人采取什么动作、哪些钱流入或流出、为什么选择这个时间窗口，以及现实可能怎样偏离模型。禁止只写「在30天压力情景下」就继续往下。
6. **类比必须标注边界。** 类比用于打开理解，不用于替代机制。说明故事中的角色分别对应什么，并指出类比在哪一步会失效。假设数字必须标为教学示例，不得伪装成真实数据。
7. **保留证据和限定。** 简化表达时保留数据年份、比较对象、评选者、方法、反方证据和不确定性。不要把「在某榜单、某方法、某一年排名第一」压缩成「它就是第一」。
8. **按固定顺序推进。** 优先使用「现场观察 → 小白问题 → 日常机制 → 正式术语 → 真实数据/来源 → 不代表什么 → 结论」；让读者先理解，再记名字。

只在后文会反复使用时保留缩写。一个术语只出现一两次，解释后优先继续使用中文含义，避免形成缩写汤。

写解释型、科普型或面向10岁读者的文章时，必须读取 `references/depth_without_jargon.md`，按其中的递归解释法和10岁读者测试执行。

## 核心博客输出规范 (Blog Formatting Rules)

当你为博客(`_posts/`)生成内容时，必须绝对遵守以下格式要求，这是生成内容的**最高执行法则**：

### 1. 双语输出机制 (Bilingual Pairs)
- 如果任务要求生成同一篇文章的中英文版本，必须生成**两个独立的文件**，绝对不能在一个Markdown文件中混排中英文。
- 使用平行的文件名命名：
  - 英文：`YYYY-MM-DD-topic-slug.md`
  - 中文：`YYYY-MM-DD-topic-slug-zh.md`
- 保持两种语言版本在核心论点、结构和案例上的对齐，但**绝不要逐句机械翻译**。中文版不是英文版的字幕轨，英文版也不是中文版的机翻。每个版本都应该读起来像是用该语言原生的思维写出来的（中文有力、自然；英文干练、易读）。
- **当你基于用户的源文本写作时：忠于原文。** 不要写得过于抽象。尽可能多地使用用户的原始表述，不要对原有的表达做过度修改。你可以重新排列句子的逻辑，但不要改变原始的措辞。不要试图去过度"润色"或"提升"原文。

### 2. 强制前置元数据 (Front Matter)
必须遵循 Jekyll 博客的 YAML 格式：`title`, `date`, `permalink`, 和 `tags`。
- 中英文版本共用同一个 `YYYY/MM/topic-slug`，靠语言前缀区分：英文 `/posts/YYYY/MM/topic-slug/`，中文 `/zh/posts/YYYY/MM/topic-slug/`。permalink 里绝不要加 `-en`、`-cn`、`-zh` 后缀。
- 标题 (`title`) 必须贴合目标语言的表达习惯，而不是生硬的对称镜像。
示例：
```yaml
---
title: 'Example Post Title Goes Here'
date: 2026-03-12
permalink: /posts/2026/03/example-post-title-goes-here/
tags:
  - Example
  - Tag
---
...
- **第一句话必须是极具吸引力的钩子（Punchy Hook）**。这句话极其重要，因为它会出现在文章列表和RSS订阅中。
  ...
  - 对于教程文章：直接点明谁能受益、能获得什么具体价值。
  - **避免**软弱无力的开头（例如 "Welcome to...", "This is a post about...", "In this article..."，"随着AI的快速发展..."）。
  - **测试标准**：这句话能让读者在列表页产生点击的冲动吗？
  - **好句示例**：
...
- 作者署名不属于写作任务。不要手写或复制署名行，它会自动生成。

### 4. 排版与Markdown细节
- 文章应该以**自然段落**组织，不要写成碎片化的短句。
- 偏好中短长度的段落，每个段落要有清晰的推进：主张 (claim) → 压力 (pressure) → 后果 (consequence) → 案例 (example) → 总结 (takeaway)。确保每段都在真正推动观点、强化框架或增加论据。
- 图片路径必须放在 `/images/<post-name>/` 下（例如 `/images/vibe-coding-ides/`）。
- 在Markdown链接中，使用带有URL编码空格（`%20`）的绝对路径。
- 不要使用 `_posts/attachments/`，Jekyll不会从该目录提供文件。
- 自然段落之间必须留出空行，以确保Markdown渲染正确。
- 在中文语境下，中英文之间要适当加空格（例如 `Cursor 学生会员`, `vibe coding 功能`）。
- **数学公式 (MathTeX) 规范**：由于 GitHub Pages (Jekyll/Kramdown) 的限制，所有的**行内公式和块级公式都必须使用双美元符号 `$$ ... $$`** 包裹，绝对不能使用单美元符号 `$ ... $`。例如：必须写成 `$$E=mc^2$$`，而不是 `$E=mc^2$`。
...
kw的文章基本可以归为五种原型，写之前先判断属于哪种，每种的写法重心不同：
...
**亲自下场**：这是kw最核心的写作基因。不是隔空评论，而是真的去做那件事。写的时候，要让读者感觉到"这个人真的做了这件事"，而不是"这个人在想象做这件事"。
...
2. **过度结构化**：不用bullet point罗列观点，不大量加粗。kw的文章绝大多数时候没有小标题，从头到尾一口气顺下来，靠节奏和转场自然推进。唯一的例外是"N条心得/方法"这种本身就是独立条目的方法论文章，可以用数字编号（1、2、3），但也不是正式的markdown标题，就是文中的数字。如果不是这种分条目的结构，就不要加小标题，用口语化的转场句（"说到这个""回到xxx这块"）来衔接板块
3. **标点禁令**：
   - 不使用冒号"："，用逗号代替
   - 不使用破折号"——"
...
kw的文章有一批高频出现的口语化表达，这些词组自带"活人感"，写作时应该主动、自然地使用：
...
【升华】从具体事件拉到更大的文化/哲学/历史参照物
  ...
  ↓
【收尾】几种常见收法，选最合适的一种：
  - 引用收尾：用别人的一句话作结（"比如说：磨平一些信息差"）
  - 哲思余韵：一个短句留白（"比如说：时间。流逝的本身。"）
  - 行动呼吁：鼓励读者去做一件事
  - 信念宣言：表达对未来的坚信
...
**L1-5 缩写与黑话检查**
- 从标题开始，按读者看到内容的顺序列出所有非日常缩写和术语，确认第一次出现的同一句或紧邻下一句就提供全称、中文名和一句话作用，没有「先用后讲」。
- 搜索定义段中的新术语，确认没有出现「用黑话解释黑话」的链条。
- 搜索所有百分比，确认附近明确写出分子、分母、时间范围和不代表什么。
- 搜索「情景」「压力测试」「模型假设」等词，确认后文列出具体事件和假设，而不是把名称当解释。
...
这一层检查文章是否符合kw的写作模式，类似代码的单元测试。每一项都给出"是/否"判断。
...
**L3-7 深度与可读性检查**
- 非专业读者能否说出每个核心指标在回答什么问题？
- 每个比例是否都能回答「谁除以谁」，并有一个清楚标注的数值翻译？
- 每个抽象情景是否交代了参与者、动作、资金或因果变化、时间窗口和模型边界？
- 简化后是否仍保留年份、比较对象、方法、来源、反例和限定条件？
- 对目标读者做复述测试：一个10岁孩子能否不查词典，用自己的话解释核心结论以及它不代表什么？

**通过标准**：L3-1、L3-2和L3-7必须全部通过，L3-3至L3-6中至少通过相关项（根据文章类型，有些项目可能不适用则跳过）。
...
**修复方式**：这一层没有机械的修复方法。核心操作是：把"AI味重"的段落找出来，想象kw本人会怎么说这段话，然后用更口语、更私人、更不完美的方式重写。
...
解释专业概念、数字、缩写和模型情景时，参考 `references/depth_without_jargon.md`。
