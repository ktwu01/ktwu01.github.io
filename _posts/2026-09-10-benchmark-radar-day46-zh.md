---
title: "Benchmark Radar 第四十六天：arXiv 发布——arXiv:2609.11115 上线与 v0.11.0 释出"
date: 2026-09-10
...
Benchmark Radar 的第四十六天。技术论文正式登上 arXiv，编号 `arXiv:2609.11115`；v0.11.0 发布携带冻结数据与经验证哈希；全项目引用面更新指向在线 arXiv 条目。记分牌：185 颗星，30 个 fork。
...
- 普查：1,283 源记录（790 有分、493 无分），12,916 数值观测
- 37 个公开源（13 直连连接器，24 第一方实验室源）

PR #591 推进论文子模块至引用修订并验证 arXiv 投稿包。PR #592 推进至源计数修订，落实 37 源拆解。PR #593 推进至重建 PDF——即提交 arXiv 的 `main.pdf`，现已入库。

PR #594 在两个 README 增加 Abstract 段，描述活体数据库、37 源每日发现、带分数历史的基准目录、含饱和度与趋势视图的排行榜、离线查询 CLI。
...
PR #596 在引用页标注分数来源，PR #597 更新贡献分。PR #598 CI 核验冻结论文数据，确认哈希匹配与普查一致。

为什么要在意

arXiv 发布让 Benchmark Radar 方法论可引用、永久可得、可被检索。v0.11.0 让证据可复现：冻结提交、固定截数、哈希数据包、CI 验证的哈希，意味着任何人都能对着同一组输入核验论文里的每个数字。

全项目引用面——README 徽章、引用页、仪表盘页脚、CLI 帮助——现在指向真实的 arXiv 编号，而非预印本占位符。论文进入学术记录。
...
第四十七天：引用面转为读者优先，Cite 卡片默认折叠 APA/BibTeX，顶部导航新增 Cite 入口。
