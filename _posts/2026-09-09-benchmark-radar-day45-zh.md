...
PR #586 为冻结论文数据加 CI 核验。工作流验证提交的 `figure-data.tex` 哈希与 v0.11.0 发布包一致、`catalog-data.tex` 与 `findings-data.tex` 符合冻结普查、重建 PDF 通过小数值扫描。确保论文的量化声明在截数后不再漂移。

PR #587 让报告测试与手稿、入库导出对齐。测试套件现在核验论文的普查数字（1,283 源记录、790 有分、493 无分、12,916 观测值）与导出的图表数据一致，审计 JSON 证据文件（`evidence/catalog-audit.json`、`evidence/catalog-findings.json`）结构完好。

PR #588 合并 agents 分支策略：动 `main` 或论文子模块指针需显式请求。PR #589 把基准自有排行榜并入报告目录，PR #590 用基准源的注册名做分数标题。
