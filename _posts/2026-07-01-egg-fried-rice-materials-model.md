...
The 2023 model treated φ(t) as a bare first-order relaxation, coagulation approaching completion smoothly and exponentially from the moment heat is applied. Real protein coagulation is denaturation, aggregation, and gelation, and no closed form is exactly right for it, but an Avrami form is a better first approximation than first-order decay, because with an exponent n > 1 it captures the nucleate-then-grow character a first-order law cannot: φ(t) = 1 − exp[−(k(T)t)ⁿ]. I take n > 1 here precisely so coagulation starts slow and accelerates rather than starting at its maximum rate; at n = 1 this reduces back to the original first-order form. The temperature dependence lives in an Arrhenius rate k(T), but with n fixed that mostly rescales the time axis: a colder wok runs the same curve stretched out, not a categorically different one. Even so, swapping in Avrami wasn't cosmetic; it changes the *shape* of how coverage builds over the cook, which changes everything downstream that depends on timing.

**The original stirring term had the sign backwards, and fixing it revealed a real optimum.**

This is the part of the 2023 conversation I'm least proud of and most glad I went back to. The original model used P = P₀e^(−I/I₀) for contact probability as a function of stirring intensity I, which means more stirring monotonically reduces egg-rice contact, with no stated mechanism for why. It also flatly contradicts kitchen experience: barely-stirred fried rice does not bond evenly, it scrambles into segregated patches. The rework instead models two competing effects that both plausibly scale with stirring intensity. Collision frequency between rice grains and egg fragments grows roughly linearly with I, more agitation means more chances to touch. But stirring is not purely constructive: it also physically tears already-bonded egg film back off the grains it's stuck to, and that shear-induced fragmentation grows roughly quadratically with I, since it depends on both how often grains collide and how hard. Those exponents (1 for collision, 2 for fragmentation) are a modeling assumption, not a derived law; what actually matters is only that fragmentation grows faster in I than collision does. Under that assumption, linear gain and quadratic loss mean fragmentation eventually wins at high I. That produces a genuine non-monotonic optimum, a specific stirring intensity I* that maximizes final coverage for a given cook time, instead of a model where the answer to "how much should I stir" is just "as little as possible" or "as much as possible." Both under-stirring and over-stirring ruining a dish, for different physical reasons, is a very ordinary cooking experience. The original model couldn't produce that. The rework does.
...
The original question was about *every* grain being bonded, and the original model only ever tracked mean coverage, ⟨θ⟩. But a population of rice grains can hit a high mean coverage while still failing the actual question: some grains completely bare, others buried in egg, averaging out to a respectable-looking number that describes no individual grain in the pan. The rework adds a working criterion for "no isolated rice grains": you need ⟨θ⟩ ≥ θc *and* low variance across grains *and* an explicit bound on the fraction of grains left bare. That bare-fraction bound is the term that actually enforces "every grain," since it's the only one a well-coated majority can't paper over. This is a heuristic acceptance criterion I'm imposing, not a necessary-and-sufficient physical definition, which is why I don't write it as an "if and only if." And it's honestly one-sided: θ tracks how well each *rice* grain is coated, so it captures "no bare rice" but not the symmetric "no isolated egg," which would need a separate variable for free egg mass. Even so, a high mean with high variance is exactly the "bald patches next to egg clumps" failure mode anyone who's rushed a stir-fry has produced.
