...
PR #387 修复了 All dates 视图。它过去会把同一条记录在每次重叠扫描里重复展示，所以 `modelscope/evalscope@v1.11.0` 因为两天的 sighting 出现了两次。现在每条来源记录只显示最新一次匹配的 sighting。单个日期视图、每日快照和趋势计算保留完整历史。
...
标题只有裸标签的发布，看起来和普通版本升级没区别。一行摘要让卡片可扫读，回填旧条目让存档保持诚实，而不是只修未来。
