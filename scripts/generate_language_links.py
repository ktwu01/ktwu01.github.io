...
#   English URL /posts/YYYY/MM/slug/     filename YYYY-MM-DD-slug.md
#   Chinese URL /zh/posts/YYYY/MM/slug/  filename YYYY-MM-DD-slug-zh.md
# `zh` is the ISO 639 language code; `cn` is a country code and is not used
# as a language marker in canonical URLs or source filenames.
...
def find_monolingual_posts(posts):
    """Return canonical posts that do not have another language variant."""
    groups = {}
    for post in posts:
        if CANONICAL_URL.fullmatch(post.permalink):
            groups.setdefault(post.pair_key, []).append(post)

    warnings = []
    for _pair_key, variants in sorted(groups.items()):
        languages = {post.language for post in variants}
        if len(languages) != 1:
            continue
        post = variants[0]
        missing_languages = sorted(set(LANGUAGE_METADATA) - languages)
        missing_labels = ", ".join(
            LANGUAGE_METADATA[language]["label"]
        ...
        warnings.append(
            (
                post,
                f"post has only {present_label}; "
                f"missing {missing_labels} translation",
            )
        )
    return warnings


def find_filename_warnings(posts):
    """Return posts whose source filename does not use the canonical language form."""
    warnings = []
    for post in posts:
        if not CANONICAL_URL.fullmatch(post.permalink):
            continue
        lower_name = post.path.name.lower()
        if post.language == "zh" and not lower_name.endswith("-zh.md"):
            if lower_name.endswith("-cn.md"):
                detail = "`-cn.md` is not the canonical language suffix"
            else:
                detail = "Chinese source filename is missing the `-zh.md` suffix"
            warnings.append(
                (post, f"{detail}; rename this source file to end in `-zh.md`")
            )
        elif post.language == "en" and lower_name.endswith(("-en.md", "-cn.md", "-zh.md")):
            warnings.append(
                (
                    post,
                    "English source filenames must not have a language suffix",
                )
            )
    return warnings


def print_language_warnings(posts):
    warnings = [
        *find_filename_warnings(posts),
        *find_monolingual_posts(posts),
    ]
    for post, message in warnings:
        # GitHub Actions renders this workflow-command syntax as an annotation.
        print(
            f"::warning file={post.path},title=Blog language convention::{message}"
        )
    if warnings:
        print(f"Blog language warning: {len(warnings)} issue(s).")
    return len(warnings)
