...
class LanguageLinkTests(unittest.TestCase):
    ...
    def test_builds_links_in_both_directions(self):
        posts = [
            ...
            self.post("2026-07-19-topic-zh.md", "/zh/posts/2026/07/topic/"),
        ]
    ...
    def test_monolingual_post_produces_coverage_warning(self):
        post = self.post("2026-07-19-topic.md", "/posts/2026/07/topic/")

        warnings = self.module.find_monolingual_posts([post])

        self.assertEqual(len(warnings), 1)
        self.assertEqual(warnings[0][0], post)
        self.assertIn("missing 中文 translation", warnings[0][1])

    def test_bilingual_pair_produces_no_coverage_warning(self):
        posts = [
            self.post("2026-07-19-topic.md", "/posts/2026/07/topic/"),
            self.post("2026-07-19-topic-zh.md", "/zh/posts/2026/07/topic/"),
        ]

        self.assertEqual(self.module.find_monolingual_posts(posts), [])

    def test_cn_filename_produces_convention_warning(self):
        post = self.post(
            "2026-07-19-topic-cn.md", "/zh/posts/2026/07/topic/"
        )

        warnings = self.module.find_filename_warnings([post])

        self.assertEqual(len(warnings), 1)
        self.assertIn("`-cn.md` is not", warnings[0][1])
        self.assertIn("`-zh.md`", warnings[0][1])

    def test_canonical_source_filenames_produce_no_warning(self):
        posts = [
            self.post("2026-07-19-topic.md", "/posts/2026/07/topic/"),
            self.post("2026-07-19-topic-zh.md", "/zh/posts/2026/07/topic/"),
        ]

        self.assertEqual(self.module.find_filename_warnings(posts), [])
    ...
    def test_rejects_filename_pair_with_mismatched_public_slugs(self):
        posts = [
            self.post("2026-07-19-topic.md", "/posts/2026/07/topic/"),
            self.post(
                "2026-07-19-topic-zh.md", "/zh/posts/2026/07/different-topic/"
            ),
        ]
    ...
    def test_check_mode_rejects_stale_output(self):
        with tempfile.TemporaryDirectory() as root:
            ...
            for name, permalink in (
                ...
                ("2026-07-19-topic-zh.md", "/zh/posts/2026/07/topic/"),
            ):
